...
export function PortfolioAssetForm({ onAdd }: PortfolioAssetFormProps) {
  ...
  return <form className="raoniForm" onSubmit={submit}>
    ...
    {form.kind !== "cash" ? <label>{form.kind === "equity" ? "매칭 종목코드" : "식별 코드"}<input value={form.symbol} onChange={(event) => setForm((current) => ({ ...current, symbol: event.target.value }))} placeholder={form.kind === "crypto" ? "BTC, USDT 또는 USDC" : "선택 사항"} />{form.kind === "crypto" ? <small>바이비트 USDT와 Hyperliquid USDC는 현물로 입력하면 자동 시세를 반영합니다.</small> : null}</label> : null}
